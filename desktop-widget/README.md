...
| 창 이동 | 위젯 **맨 위 28px 손잡이 줄**(가운데 알약 표시)을 잡고 드래그 |
| 닫기(X) | 종료가 아니라 트레이로 숨김 |
| 트레이 아이콘 | 보이기/숨기기 · 화면 오른쪽 위로 이동 · 로그인 시 자동 실행 토글 · 종료 |
...
└── src-tauri/
    ...
    ├── capabilities/       # 원격 페이지(plna.vercel.app, localhost)에 드래그 권한 부여
...
제목표시줄이 없는 창이라 잡을 곳을 페이지가 만들어 줘야 한다. 손잡이는
**웹 앱(`components/widget/widget-drag-bar.tsx`)이 직접 그린다.** 앱을 다시
설치하지 않아도 배포만 하면 손잡이가 나타난다.

| 층 | 하는 일 |
|----|---------|
| 앱 → 페이지 | 위젯 주소에 `?desktop=1` 을 붙이고, 초기화 스크립트가 `<html>` 에 `data-plna-desktop-widget` 표시를 남긴다 |
| 페이지 | 위 신호(또는 `window.__TAURI_INTERNALS__`)를 보면 맨 위에 `data-tauri-drag-region` 줄을 그린다 |
| 앱 | 그 줄에서 시작된 mousedown 을 받아 창을 끈다. 페이지가 손잡이를 안 그리면(구버전 배포) 스크립트가 대신 하나 얹는다 |

브라우저 탭이나 폰 홈 화면에서 `/widget` 을 열면 신호가 없으므로 손잡이는 나오지 않는다.

**손잡이가 안 보이거나 창이 화면 밖으로 나갔다면** 트레이 아이콘 메뉴의
**"화면 오른쪽 위로 이동"** 을 누른다. 드래그와 무관하게 창을 제자리로 돌려놓는다.
