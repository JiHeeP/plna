...
function buildPrompt(
  ...
  const timeScope = period === "weekly" ? "이번 주(월~일)" : "이번 달";

  return `너는 나의 가까운 친구이자 목표 실행 파트너야. 아래 ${periodLabel} 데이터를 보고 친구한테 말하듯이 편하게 인사이트를 줘.

${context}

중요 규칙:
1. 일지에 적은 내용을 그대로 복붙하지 마. ${timeScope} 전체 기록 + 이번 달 월간 목표를 종합해서 의미 있는 패턴이나 흐름을 찾아서 말해줘.
2. 반드시 월간 목표와 연결해서 이야기해줘. 월간 목표 달성에 어떻게 기여했는지, 또는 어떤 부분이 부족한지.
3. 가장 높은 달성률을 보인 항목이 있으면 "굳!" 같은 느낌으로 칭찬해줘.
4. surpriseInsight에는 데이터에서 발견한 의외의 패턴이나 내가 미처 인식하지 못했을 연결고리를 하나 넣어줘.
5. 친구처럼 편하게 말하되 반말로 해줘. 예: "이번 주 운동 루틴 완전 자리잡았네, 굳!" / "여기는 좀 아쉬운데~"

아래 JSON 형식으로만 응답 (다른 텍스트 없이):
{
  "wentWell": [
    "${timeScope} 종합 잘한 점 1 (1-2문장, 월간 목표와 연결)",
    "${timeScope} 종합 잘한 점 2 (1-2문장, 월간 목표와 연결)",
    "${timeScope} 종합 잘한 점 3 (1-2문장, 월간 목표와 연결)"
  ],
  "toImprove": [
    "${timeScope} 종합 보완할 점 1 (1-2문장, 월간 목표와 연결)",
    "${timeScope} 종합 보완할 점 2 (1-2문장, 월간 목표와 연결)",
    "${timeScope} 종합 보완할 점 3 (1-2문장, 월간 목표와 연결)"
  ],
  "nextFocus": [
    "6개월 마일스톤 + 월간 목표 기반 행동 포커스 1 (1-2문장)",
    "6개월 마일스톤 + 월간 목표 기반 행동 포커스 2 (1-2문장)",
    "6개월 마일스톤 + 월간 목표 기반 행동 포커스 3 (1-2문장)"
  ],
  "surpriseInsight": "데이터에서 발견한 의외의 패턴이나 숨겨진 연결고리 하나 (1-2문장)"
}

규칙:
- 한국어 반말 (친구 톤)
- 각 필드(wentWell, toImprove, nextFocus)는 반드시 3개 문자열 배열
- surpriseInsight는 문자열 1개
- 일지 내용 그대로 쓰지 말고, 주간/월간 단위로 합쳐서 의미 있게 종합
- 달성률 높은 항목은 "굳!" 느낌으로 칭찬
...
async function generateAiText(
  ...
  try {
        ...
        messages: [
          {
            role: "system",
            content:
              "너는 사용자의 가까운 친구이자 목표 실행 파트너야. 습관, 할 일, 마일스톤, 월간 목표, 저널 데이터를 종합 분석하여 친구한테 말하듯 편한 반말 톤으로 인사이트를 JSON으로만 답해. 일지 내용을 그대로 복붙하지 말고 종합해서 의미 있는 패턴을 찾아. 각 필드(wentWell, toImprove, nextFocus)는 반드시 3개 문자열 배열, surpriseInsight는 문자열 1개.",
          },
    ...
    const surpriseInsight =
      typeof parsed.surpriseInsight === "string" && parsed.surpriseInsight.trim()
        ? parsed.surpriseInsight.trim()
        : undefined;

    return { wentWell, toImprove, nextFocus, surpriseInsight };
  } catch (err) {
...
async function buildSnapshot(
  ...
  if (aiText) {
    return {
      ...
      surpriseInsight: aiText.surpriseInsight,
